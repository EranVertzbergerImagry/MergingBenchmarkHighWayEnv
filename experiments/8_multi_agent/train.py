"""
Social Attention DQN — Experiment 8: Multi-Agent Intersection

Extended from experiment 7 with:
  - 4 controlled vehicles with staggered random spawning
  - Per-agent evaluation logging
  - Shared DQN policy (same weights, independent observations)
...
# ---------------------------------------------------------------------------
# Multi-agent attention display (no decision pane)
# ---------------------------------------------------------------------------
# One color per agent slot (RGBA base — alpha set dynamically)
AGENT_COLORS = [
...
def _compute_attention_for_state(agent, single_state):
    """Run forward pass on one agent's obs and return (attention_matrix, mask)."""
    import torch
    ...
    # shape: (1, heads, 1, vehicles) → squeeze to (heads, vehicles)
    attention = attention.squeeze(0).squeeze(1).detach().cpu().numpy()
    _, _, mask = agent.value_net.split_input(state_t)
    mask = mask.squeeze()  # (vehicles,)
    return attention, mask


def _match_state_to_vehicles(single_state, mask, obs_type, road_vehicles, ego_vehicle):
    """Map observation rows to actual Vehicle objects using position proximity.

    The state is ego-centric (rotated by -ego_heading, translated to ego origin,
    then normalized). We reverse these transforms to get world positions for matching.
    """
    from rl_agents.utils import remap

    # Ego heading for reverse rotation: R(+theta) is the inverse of R(-theta)
    ...
    for v_index in range(single_state.shape[0]):
        ...
        # Un-normalize from [-1, 1] to feature range
        ...
        # Reverse ego-centric rotation: R(+theta) * [ex, ey]
...
def multi_agent_attention_display(agent, agent_surface, sim_surface):
    """Draw per-agent attention overlays on the sim surface. No decision pane."""
    import pygame
    # Black out the agent_surface (no decision pane)
    pygame.draw.rect(agent_surface, (0, 0, 0),
                     (0, 0, agent_surface.get_width(), agent_surface.get_height()), 0)

    # Use _multi_agent_state (saved before act() clobbers previous_state)
    # Fall back to previous_state for single-agent compatibility
    state = getattr(agent, '_multi_agent_state', None) or agent.previous_state
    if state is None:
        return
    ...
    for agent_idx in range(n_agents):
        ...
        # Skip unborn/done agents (all-zero observations)
        if single_state.sum() == 0:
            continue

        # Need the vehicle for this agent
        if not (hasattr(env, '_agent_spawned')
                and agent_idx < len(env._agent_spawned)
...
# ---------------------------------------------------------------------------
# Multi-agent record() patch
# ---------------------------------------------------------------------------
def _patch_record_for_multi_agent(agent):
    """Monkey-patch agent.record() to use per-agent rewards from info.

    The default rl-agents record() passes the same scalar reward to every
    agent's replay buffer entry. This patch:
      - Reads per-agent rewards from info["agent_rewards"]
      - Skips unspawned agents (all-zero observations)
    """
    original_record = agent.record.__func__  # unbound method
    ...
    def patched_record(self, state, action, reward, next_state, done, info):
        ...
        if isinstance(state, tuple) and isinstance(action, tuple):
            ...
            for i, (s, a, ns) in enumerate(
                ...
                # Skip unspawned agents (zero observations)
                if s.sum() == 0:
                    continue
...
# Train
# ---------------------------------------------------------------------------
def train(config, run_dir, visualize=False, initial_model=None):
...
def run_episodes(agent, env, num_episodes):
    ...
    for ep in range(num_episodes):
        ...
        total_reward = 0.0
        agent_reward_sums = None  # per-agent accumulator for multi-agent
        steps = 0
        ...
        while not done:
            ...
            total_reward += reward
            # Accumulate per-agent rewards from info
...
def evaluate_agent(agent, env_config, run_dir, num_episodes, eval_csv=None):
    ...
    # Per-episode average reward (across spawned agents)
...
def demo(agent, env_config, num_episodes):
    ...
    EnvViewer.agent_display = lambda a_surf, s_surf: multi_agent_attention_display(agent, a_surf, s_surf)

    # Fixed camera at intersection center for multi-agent view
    ...
    for ep in range(num_episodes):
        ...
        # Pin camera to intersection center after viewer is created
        try:
        ...
        steps = 0
        while not done:
            # Save tuple obs before act() overwrites agent.previous_state
        ...
        if ep == 0:
            try:
                env.unwrapped.viewer.set_agent_display(
                    lambda a_surf, s_surf: multi_agent_attention_display(agent, a_surf, s_surf)
                )
            except AttributeError:
...
def main():
    parser = argparse.ArgumentParser(
        description="Social Attention DQN — Experiment 8: Multi-Agent"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to a .tar checkpoint to warm-start training from",
    )
