...
def generate_training_plot(episodes_csv, plot_path, description="", window=50,
    ...
    Handles multi-agent CSVs (multiple rows per episode) by aggregating
    per-episode: average reward, max length.
    ...
    # Read and aggregate per-episode (handles multi-agent duplicate rows)
    ep_data = defaultdict(lambda: {"rewards": [], "lengths": []})
    try:
        with open(episodes_csv) as f:
            reader = csv.DictReader(f)
            for row in reader:
                ep = int(row["episode"])
                ep_data[ep]["rewards"].append(float(row["reward"]))
                ep_data[ep]["lengths"].append(int(row["length"]))
    ...
    # Average reward across agents, length is shared (take max)
    rewards = np.array([np.mean(ep_data[e]["rewards"]) for e in sorted_eps])
    lengths = np.array([max(ep_data[e]["lengths"]) for e in sorted_eps])
